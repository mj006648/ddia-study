...
- **Aggregate 경계:** 함께 읽고 함께 쓰는 데이터 묶음이 명확하면 문서 모델을 고려한다.
- **관계 밀도:** 다대다 관계와 ad-hoc query가 많으면 관계형 모델이 안전하다.
- **탐색 깊이:** 여러 단계의 관계 탐색이 핵심이면 그래프 모델을 고려한다.
- **중복 허용:** 읽기 최적화를 위해 중복 저장할 때, 업데이트 전파 주체와 실패 시 복구 방식을 정한다.

## 11. 미니 사례

데이터셋 카탈로그를 만든다면 `dataset`, `owner`, `permission`, `project`는 관계형 테이블로 관리하기 좋다. 반면 데이터셋 설명, 생성 조건, 품질 지표, 도메인별 태그는 문서형 메타데이터로 확장하기 좋다. 데이터셋이 어떤 workflow, 산출물, 모델, 논문과 연결되는지는 그래프 구조로 표현하면 lineage 질의가 쉬워진다.

## 12. 한 줄 결론

데이터 모델은 저장소 선택이 아니라 애플리케이션의 사고방식이며, 여러 모델을 함께 쓸 때는 원천과 파생의 경계를 반드시 정해야 한다.
